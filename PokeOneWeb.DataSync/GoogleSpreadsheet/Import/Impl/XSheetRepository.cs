...
{
    public abstract class XSheetRepository<TDto, TEntity>
        : XISheetRepository
        where TEntity : class, IHashedEntity, IEntity
        where TDto : XISpreadsheetEntityDto
    {
        protected readonly ApplicationDbContext DbContext;
        protected readonly ISpreadsheetImportReporter Reporter;
        private readonly XISheetRowParser<TDto> _parser;
        private readonly XISpreadsheetEntityMapper<TDto, TEntity> _mapper;

        protected XSheetRepository(
            ApplicationDbContext dbContext,
            XISheetRowParser<TDto> parser,
            XISpreadsheetEntityMapper<TDto, TEntity> mapper,
        ...
        public List<RowHash> ReadDbHashes(ImportSheet sheet)
        {
            var rowHashes = DbSet
                ...
                .Select(x => new RowHash { Hash = x.Hash, IdHash = x.IdHash, ImportSheetId = sheet.Id })
                .ToList();
