// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using PokeOneWeb.Data;

#nullable disable

namespace PokeOneWeb.Data.Migrations.ReadModelDbMigrations
{
    [DbContext(typeof(ReadModelDbContext))]
    [Migration("20220422141223_extend-location-group")]
    partial class extendlocationgroup
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.2")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.AttackEffectivityReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int?>("BuildReadModelId")
                        .HasColumnType("int");

                    b.Property<decimal>("Effectivity")
                        .HasColumnType("decimal(5,2)");

                    b.Property<int?>("PokemonVarietyReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("TypeName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("BuildReadModelId");

                    b.HasIndex("PokemonVarietyReadModelId");

                    b.ToTable("AttackEffectivityReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.BuildReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Ability")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("AbilityDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<int>("AtkEv")
                        .HasColumnType("int");

                    b.Property<string>("BuildDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BuildName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("DefEv")
                        .HasColumnType("int");

                    b.Property<int>("HpEv")
                        .HasColumnType("int");

                    b.Property<string>("PokemonName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PokemonResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PokemonVarietyReadModelId")
                        .HasColumnType("int");

                    b.Property<int>("SpaEv")
                        .HasColumnType("int");

                    b.Property<int>("SpdEv")
                        .HasColumnType("int");

                    b.Property<int>("SpeEv")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.HasIndex("PokemonVarietyReadModelId");

                    b.ToTable("BuildReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.EntityTypeReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("EntityType")
                        .HasColumnType("int");

                    b.Property<string>("ResourceName")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("ResourceName")
                        .IsUnique()
                        .HasFilter("[ResourceName] IS NOT NULL");

                    b.ToTable("EntityTypeReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.EvolutionAbilityReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("AbilityName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PokemonName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PokemonResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PokemonSortIndex")
                        .HasColumnType("int");

                    b.Property<int?>("PokemonVarietyAsHiddenAbilityId")
                        .HasColumnType("int");

                    b.Property<int?>("PokemonVarietyAsPrimaryAbilityId")
                        .HasColumnType("int");

                    b.Property<int?>("PokemonVarietyAsSecondaryAbilityId")
                        .HasColumnType("int");

                    b.Property<int>("RelativeStageIndex")
                        .HasColumnType("int");

                    b.Property<string>("SpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("PokemonVarietyAsHiddenAbilityId");

                    b.HasIndex("PokemonVarietyAsPrimaryAbilityId");

                    b.HasIndex("PokemonVarietyAsSecondaryAbilityId");

                    b.ToTable("EvolutionAbilityReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.EvolutionReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<string>("BaseName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BasePrimaryElementalType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BaseResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("BaseSecondaryElementalType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("BaseSortIndex")
                        .HasColumnType("int");

                    b.Property<string>("BaseSpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("BaseStage")
                        .HasColumnType("int");

                    b.Property<string>("EvolutionTrigger")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EvolvedName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EvolvedPrimaryElementalType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EvolvedResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EvolvedSecondaryElementalType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("EvolvedSortIndex")
                        .HasColumnType("int");

                    b.Property<string>("EvolvedSpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("EvolvedStage")
                        .HasColumnType("int");

                    b.Property<bool>("IsAvailable")
                        .HasColumnType("bit");

                    b.Property<bool>("IsReversible")
                        .HasColumnType("bit");

                    b.Property<int?>("PokemonVarietyReadModelId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId");

                    b.HasIndex("PokemonVarietyReadModelId");

                    b.ToTable("EvolutionReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.HuntingConfigurationReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Ability")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<string>("Nature")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NatureEffect")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PokemonName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PokemonResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PokemonVarietyReadModelId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId");

                    b.HasIndex("PokemonVarietyReadModelId");

                    b.ToTable("HuntingConfigurationReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.ItemOptionReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<int?>("BuildReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("ItemName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ItemResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.HasIndex("BuildReadModelId");

                    b.ToTable("ItemOptionReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.ItemReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<string>("BagCategoryName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("BagCategorySortIndex")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Effect")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsAvailable")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ResourceName")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("SortIndex")
                        .HasColumnType("int");

                    b.Property<string>("SpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.HasIndex("ResourceName")
                        .IsUnique()
                        .HasFilter("[ResourceName] IS NOT NULL");

                    b.ToTable("ItemReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.ItemStatBoostPokemonReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<decimal>("AttackBoost")
                        .HasColumnType("decimal(6,2)");

                    b.Property<decimal>("DefenseBoost")
                        .HasColumnType("decimal(6,2)");

                    b.Property<bool>("HasRequiredPokemon")
                        .HasColumnType("bit");

                    b.Property<decimal>("HitPointsBoost")
                        .HasColumnType("decimal(6,2)");

                    b.Property<string>("ItemEffect")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ItemName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ItemResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RequiredPokemonName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RequiredPokemonResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("SpecialAttackBoost")
                        .HasColumnType("decimal(6,2)");

                    b.Property<decimal>("SpecialDefenseBoost")
                        .HasColumnType("decimal(6,2)");

                    b.Property<decimal>("SpeedBoost")
                        .HasColumnType("decimal(6,2)");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.ToTable("ItemStatBoostPokemonReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.LearnableMoveReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("Accuracy")
                        .HasColumnType("int");

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<int>("AttackPower")
                        .HasColumnType("int");

                    b.Property<string>("DamageClass")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EffectDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("EffectivePower")
                        .HasColumnType("int");

                    b.Property<string>("ElementalType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("HasStab")
                        .HasColumnType("bit");

                    b.Property<bool>("IsAvailable")
                        .HasColumnType("bit");

                    b.Property<string>("MoveName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PokemonVarietyReadModelId")
                        .HasColumnType("int");

                    b.Property<int>("PowerPoints")
                        .HasColumnType("int");

                    b.Property<int>("Priority")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.HasIndex("PokemonVarietyReadModelId");

                    b.ToTable("LearnableMoveReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.LearnMethodReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsAvailable")
                        .HasColumnType("bit");

                    b.Property<string>("LearnMethodName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("LearnableMoveReadModelId")
                        .HasColumnType("int");

                    b.Property<int>("SortIndex")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("LearnableMoveReadModelId");

                    b.ToTable("LearnMethodReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.LocationGroupReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<string>("EventEndDate")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EventName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EventStartDate")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsEventRegion")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NextLocationGroupName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NextLocationGroupResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PreviousLocationGroupName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PreviousLocationGroupResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RegionName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RegionResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SortIndex")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("LocationGroupReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.LocationReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<bool>("IsDiscoverable")
                        .HasColumnType("bit");

                    b.Property<int?>("LocationGroupReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Notes")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SortIndex")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("LocationGroupReadModelId");

                    b.ToTable("LocationReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.MoveOptionReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("Accuracy")
                        .HasColumnType("int");

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<int>("AttackPower")
                        .HasColumnType("int");

                    b.Property<int?>("BuildReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("DamageClass")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EffectDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ElementalType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("MoveName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PowerPoints")
                        .HasColumnType("int");

                    b.Property<int>("Priority")
                        .HasColumnType("int");

                    b.Property<int>("Slot")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.HasIndex("BuildReadModelId");

                    b.ToTable("MoveOptionReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.MoveReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("Accuracy")
                        .HasColumnType("int");

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<int>("AttackPower")
                        .HasColumnType("int");

                    b.Property<string>("DamageClass")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EffectDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ElementalType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PowerPoints")
                        .HasColumnType("int");

                    b.Property<int>("Priority")
                        .HasColumnType("int");

                    b.Property<string>("ResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.ToTable("MoveReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.NatureOptionReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<int?>("BuildReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("NatureEffect")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NatureName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.HasIndex("BuildReadModelId");

                    b.ToTable("NatureOptionReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.NatureReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<int>("Attack")
                        .HasColumnType("int");

                    b.Property<int>("Defense")
                        .HasColumnType("int");

                    b.Property<string>("Effect")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SpecialAttack")
                        .HasColumnType("int");

                    b.Property<int>("SpecialDefense")
                        .HasColumnType("int");

                    b.Property<int>("Speed")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.ToTable("NatureReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.PlacedItemReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<int>("Index")
                        .HasColumnType("int");

                    b.Property<bool>("IsConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("IsHidden")
                        .HasColumnType("bit");

                    b.Property<string>("ItemName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("ItemReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("ItemResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ItemSpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LocationName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("LocationReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("LocationResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("LocationSortIndex")
                        .HasColumnType("int");

                    b.Property<string>("Notes")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PlacementDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Quantity")
                        .HasColumnType("int");

                    b.Property<string>("RegionColor")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RegionName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Screenshot")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SortIndex")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.HasIndex("ItemReadModelId");

                    b.HasIndex("LocationReadModelId");

                    b.ToTable("PlacedItemReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.PokemonVarietyFormReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Availability")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PokemonVarietyReadModelId")
                        .HasColumnType("int");

                    b.Property<int>("SortIndex")
                        .HasColumnType("int");

                    b.Property<string>("SpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("PokemonVarietyReadModelId");

                    b.ToTable("PokemonVarietyFormReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<int>("Attack")
                        .HasColumnType("int");

                    b.Property<int>("AttackEv")
                        .HasColumnType("int");

                    b.Property<string>("Availability")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("AvailabilityDescription")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("CatchRate")
                        .HasColumnType("int");

                    b.Property<int>("Defense")
                        .HasColumnType("int");

                    b.Property<int>("DefenseEv")
                        .HasColumnType("int");

                    b.Property<int>("EggCycles")
                        .HasColumnType("int");

                    b.Property<int>("ExpYield")
                        .HasColumnType("int");

                    b.Property<decimal>("FemaleRatio")
                        .HasColumnType("decimal(6,2)");

                    b.Property<int>("Generation")
                        .HasColumnType("int");

                    b.Property<bool>("HasGender")
                        .HasColumnType("bit");

                    b.Property<decimal>("Height")
                        .HasColumnType("decimal(6,2)");

                    b.Property<string>("HiddenAbility")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("HiddenAbilityAttackBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<string>("HiddenAbilityBoostConditions")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("HiddenAbilityDefenseBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<string>("HiddenAbilityEffect")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("HiddenAbilityHitPointsBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<decimal>("HiddenAbilitySpecialAttackBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<decimal>("HiddenAbilitySpecialDefenseBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<decimal>("HiddenAbilitySpeedBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<int>("HitPoints")
                        .HasColumnType("int");

                    b.Property<int>("HitPointsEv")
                        .HasColumnType("int");

                    b.Property<bool>("IsFullyEvolved")
                        .HasColumnType("bit");

                    b.Property<bool>("IsMega")
                        .HasColumnType("bit");

                    b.Property<decimal>("MaleRatio")
                        .HasColumnType("decimal(6,2)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("NextPokemonName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NextPokemonResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("NextPokemonSpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Notes")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PokedexNumber")
                        .HasColumnType("int");

                    b.Property<string>("PreviousPokemonName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PreviousPokemonResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PreviousPokemonSpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PrimaryAbility")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("PrimaryAbilityAttackBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<string>("PrimaryAbilityBoostConditions")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("PrimaryAbilityDefenseBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<string>("PrimaryAbilityEffect")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("PrimaryAbilityHitPointsBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<decimal>("PrimaryAbilitySpecialAttackBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<decimal>("PrimaryAbilitySpecialDefenseBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<decimal>("PrimaryAbilitySpeedBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<string>("PrimaryElementalType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PvpTier")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PvpTierSortIndex")
                        .HasColumnType("int");

                    b.Property<string>("ResourceName")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("SecondaryAbility")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("SecondaryAbilityAttackBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<string>("SecondaryAbilityBoostConditions")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("SecondaryAbilityDefenseBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<string>("SecondaryAbilityEffect")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("SecondaryAbilityHitPointsBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<decimal>("SecondaryAbilitySpecialAttackBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<decimal>("SecondaryAbilitySpecialDefenseBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<decimal>("SecondaryAbilitySpeedBoost")
                        .HasColumnType("decimal(4,1)");

                    b.Property<string>("SecondaryElementalType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SortIndex")
                        .HasColumnType("int");

                    b.Property<int>("SpecialAttack")
                        .HasColumnType("int");

                    b.Property<int>("SpecialAttackEv")
                        .HasColumnType("int");

                    b.Property<int>("SpecialDefense")
                        .HasColumnType("int");

                    b.Property<int>("SpecialDefenseEv")
                        .HasColumnType("int");

                    b.Property<int>("Speed")
                        .HasColumnType("int");

                    b.Property<int>("SpeedEv")
                        .HasColumnType("int");

                    b.Property<string>("SpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("Weight")
                        .HasColumnType("decimal(6,2)");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.HasIndex("Name")
                        .IsUnique()
                        .HasFilter("[Name] IS NOT NULL");

                    b.HasIndex("ResourceName")
                        .IsUnique()
                        .HasFilter("[ResourceName] IS NOT NULL");

                    b.ToTable("PokemonVarietyReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.PokemonVarietyUrlReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PokemonVarietyReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("Url")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.HasIndex("PokemonVarietyReadModelId");

                    b.ToTable("PokemonVarietyUrlReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.PokemonVarietyVarietyReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Availability")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PokemonVarietyReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("PrimaryType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("SecondaryType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SortIndex")
                        .HasColumnType("int");

                    b.Property<string>("SpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("PokemonVarietyReadModelId");

                    b.ToTable("PokemonVarietyVarietyReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.RegionReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<string>("Color")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("EventEndDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("EventName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("EventStartDate")
                        .HasColumnType("datetime2");

                    b.Property<bool>("IsEventRegion")
                        .HasColumnType("bit");

                    b.Property<bool>("IsMainRegion")
                        .HasColumnType("bit");

                    b.Property<bool>("IsReleased")
                        .HasColumnType("bit");

                    b.Property<bool>("IsSideRegion")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.ToTable("RegionReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.SeasonReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Abbreviation")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Color")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SortIndex")
                        .HasColumnType("int");

                    b.Property<int?>("SpawnReadModelId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("SpawnReadModelId");

                    b.ToTable("SeasonReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.SimpleLearnableMoveReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<string>("MoveResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PokemonVarietyApplicationDbId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ApplicationDbId")
                        .IsUnique();

                    b.ToTable("SimpleLearnableMoveReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.SpawnReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("ApplicationDbId")
                        .HasColumnType("int");

                    b.Property<string>("EventEndDate")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EventName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("EventStartDate")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("HighestLevel")
                        .HasColumnType("int");

                    b.Property<bool>("IsEvent")
                        .HasColumnType("bit");

                    b.Property<bool>("IsInfinite")
                        .HasColumnType("bit");

                    b.Property<bool>("IsSyncable")
                        .HasColumnType("bit");

                    b.Property<string>("LocationName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("LocationReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("LocationResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("LocationSortIndex")
                        .HasColumnType("int");

                    b.Property<int>("LowestLevel")
                        .HasColumnType("int");

                    b.Property<string>("Notes")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("PokemonFormSortIndex")
                        .HasColumnType("int");

                    b.Property<string>("PokemonName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PokemonResourceName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("PokemonVarietyReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("RarityString")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("RarityValue")
                        .HasColumnType("decimal(18,4)");

                    b.Property<string>("RegionColor")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RegionName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("SpawnType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("SpawnTypeColor")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SpawnTypeSortIndex")
                        .HasColumnType("int");

                    b.Property<string>("SpriteName")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("LocationReadModelId");

                    b.HasIndex("PokemonVarietyReadModelId");

                    b.ToTable("SpawnReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.TimeOfDayReadModel", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Abbreviation")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Color")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SortIndex")
                        .HasColumnType("int");

                    b.Property<int?>("SpawnReadModelId")
                        .HasColumnType("int");

                    b.Property<string>("Times")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("SpawnReadModelId");

                    b.ToTable("TimeOfDayReadModel");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.AttackEffectivityReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.BuildReadModel", null)
                        .WithMany("OffensiveCoverage")
                        .HasForeignKey("BuildReadModelId");

                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("DefenseAttackEffectivities")
                        .HasForeignKey("PokemonVarietyReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.BuildReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("Builds")
                        .HasForeignKey("PokemonVarietyReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.EvolutionAbilityReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("HiddenEvolutionAbilities")
                        .HasForeignKey("PokemonVarietyAsHiddenAbilityId")
                        .OnDelete(DeleteBehavior.ClientNoAction);

                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("PrimaryEvolutionAbilities")
                        .HasForeignKey("PokemonVarietyAsPrimaryAbilityId")
                        .OnDelete(DeleteBehavior.Cascade);

                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("SecondaryEvolutionAbilities")
                        .HasForeignKey("PokemonVarietyAsSecondaryAbilityId")
                        .OnDelete(DeleteBehavior.ClientNoAction);
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.EvolutionReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("Evolutions")
                        .HasForeignKey("PokemonVarietyReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.HuntingConfigurationReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("HuntingConfigurations")
                        .HasForeignKey("PokemonVarietyReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.ItemOptionReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.BuildReadModel", null)
                        .WithMany("ItemOptions")
                        .HasForeignKey("BuildReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.LearnableMoveReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("LearnableMoves")
                        .HasForeignKey("PokemonVarietyReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.LearnMethodReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.LearnableMoveReadModel", null)
                        .WithMany("LearnMethods")
                        .HasForeignKey("LearnableMoveReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.LocationReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.LocationGroupReadModel", null)
                        .WithMany("Locations")
                        .HasForeignKey("LocationGroupReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.MoveOptionReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.BuildReadModel", null)
                        .WithMany("MoveOptions")
                        .HasForeignKey("BuildReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.NatureOptionReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.BuildReadModel", null)
                        .WithMany("NatureOptions")
                        .HasForeignKey("BuildReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.PlacedItemReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.ItemReadModel", null)
                        .WithMany("PlacedItems")
                        .HasForeignKey("ItemReadModelId");

                    b.HasOne("PokeOneWeb.Data.ReadModels.LocationReadModel", null)
                        .WithMany("PlacedItems")
                        .HasForeignKey("LocationReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.PokemonVarietyFormReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("Forms")
                        .HasForeignKey("PokemonVarietyReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.PokemonVarietyUrlReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("Urls")
                        .HasForeignKey("PokemonVarietyReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.PokemonVarietyVarietyReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("Varieties")
                        .HasForeignKey("PokemonVarietyReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.SeasonReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.SpawnReadModel", null)
                        .WithMany("Seasons")
                        .HasForeignKey("SpawnReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.SpawnReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.LocationReadModel", null)
                        .WithMany("Spawns")
                        .HasForeignKey("LocationReadModelId");

                    b.HasOne("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", null)
                        .WithMany("Spawns")
                        .HasForeignKey("PokemonVarietyReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.TimeOfDayReadModel", b =>
                {
                    b.HasOne("PokeOneWeb.Data.ReadModels.SpawnReadModel", null)
                        .WithMany("TimesOfDay")
                        .HasForeignKey("SpawnReadModelId");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.BuildReadModel", b =>
                {
                    b.Navigation("ItemOptions");

                    b.Navigation("MoveOptions");

                    b.Navigation("NatureOptions");

                    b.Navigation("OffensiveCoverage");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.ItemReadModel", b =>
                {
                    b.Navigation("PlacedItems");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.LearnableMoveReadModel", b =>
                {
                    b.Navigation("LearnMethods");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.LocationGroupReadModel", b =>
                {
                    b.Navigation("Locations");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.LocationReadModel", b =>
                {
                    b.Navigation("PlacedItems");

                    b.Navigation("Spawns");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.PokemonVarietyReadModel", b =>
                {
                    b.Navigation("Builds");

                    b.Navigation("DefenseAttackEffectivities");

                    b.Navigation("Evolutions");

                    b.Navigation("Forms");

                    b.Navigation("HiddenEvolutionAbilities");

                    b.Navigation("HuntingConfigurations");

                    b.Navigation("LearnableMoves");

                    b.Navigation("PrimaryEvolutionAbilities");

                    b.Navigation("SecondaryEvolutionAbilities");

                    b.Navigation("Spawns");

                    b.Navigation("Urls");

                    b.Navigation("Varieties");
                });

            modelBuilder.Entity("PokeOneWeb.Data.ReadModels.SpawnReadModel", b =>
                {
                    b.Navigation("Seasons");

                    b.Navigation("TimesOfDay");
                });
#pragma warning restore 612, 618
        }
    }
}
