...
import {
    EffecticeStatsModel as EffectiveStatsModel,
    IEffectiveStatsModel,
} from '../models/effective-stats.model';
...
export class StatsService {
    public getEffectiveStats(
        model: IStatsConfigurationModel
    ): IEffectiveStatsModel {
        let stats = new EffectiveStatsModel();

        stats.baseStatsExtent = this.getBaseStatsExtent(model);
        stats.baseStatsSubtotal = this.getBaseStatsSubTotal(
            stats.baseStatsExtent
        );

        stats.ivExtent = this.getIvExtent(model);
        stats.ivSubTotal = this.getIvSubTotal(
            stats.baseStatsSubtotal,
            stats.ivExtent
        );
        ...
        stats.natureSubTotal = this.getNatureSubTotal(
            stats.evSubTotal,
            stats.natureExtent
        );

        stats.abilityExtent = this.getAbilityExtent(
            model,
            stats.natureSubTotal
        );
        stats.abilitySubTotal = this.getAbilitySubTotal(
            stats.natureSubTotal,
            stats.abilityExtent
        );

        stats.itemExtent = this.getItemExtent(model, stats.abilitySubTotal);
        stats.itemSubTotal = this.getItemSubTotal(
            stats.abilitySubTotal,
            stats.itemExtent
        );

        stats.statModifierExtent = this.getStatModifiersExtent(
            model,
            stats.itemSubTotal
        );
        ...
        stats.fieldBoostsExtent = this.getFieldBoostsExtent(
            model,
    ...
    private getNatureExtent(
        model: IStatsConfigurationModel,
        subTotal: IStatsModel
    ): IStatsModel {
        if (!model.nature) {
    ...
    private getAbilityExtent(
        ...
        return <IStatsModel>{
            atk: this.getAbilityPartialValue(
                model.ability.attackBoost,
                subTotal.atk
            ),
            spa: this.getAbilityPartialValue(
                model.ability.specialAttackBoost,
                subTotal.spa
            ),
            def: this.getAbilityPartialValue(
                model.ability.defenseBoost,
                subTotal.def
            ),
            spd: this.getAbilityPartialValue(
                model.ability.specialDefenseBoost,
                subTotal.spd
            ),
            spe: this.getAbilityPartialValue(
                model.ability.speedBoost,
                subTotal.spe
            ),
            hp: this.getAbilityPartialValue(
                model.ability.hitPointsBoost,
                subTotal.spe
            ),
        };
    }

    private getItemExtent(
        model: IStatsConfigurationModel,
        subTotal: IStatsModel
    ): IStatsModel {
        if (!model.item) {
        ...
        return <IStatsModel>{
            ...
            spa: this.getItemPartialValue(
                model.item.specialAttackBoost,
                subTotal.spa
            ),
            def: this.getItemPartialValue(
                model.item.defenseBoost,
                subTotal.def
            ),
            spd: this.getItemPartialValue(
                model.item.specialDefenseBoost,
                subTotal.spd
            ),
            spe: this.getItemPartialValue(model.item.speedBoost, subTotal.spe),
            hp: this.getItemPartialValue(
                model.item.hitPointsBoost,
                subTotal.spe
            ),
        };
    }

    private getStatModifiersExtent(
        model: IStatsConfigurationModel,
        subTotal: IStatsModel
    ): IStatsModel {
        return <IStatsModel>{
            atk: this.getStatModifierPartialValue(
                model.statModifiers.atk,
                subTotal.atk
            ),
            spa: this.getStatModifierPartialValue(
                model.statModifiers.spa,
                subTotal.spa
            ),
            def: this.getStatModifierPartialValue(
                model.statModifiers.def,
                subTotal.def
            ),
            spd: this.getStatModifierPartialValue(
                model.statModifiers.spd,
                subTotal.spd
            ),
            spe: this.getStatModifierPartialValue(
                model.statModifiers.spe,
                subTotal.spe
            ),
            hp: 0,
        };
    ...
    private getFieldBoostsExtent(
        model: IStatsConfigurationModel,
        subTotal: IStatsModel
    ): IStatsModel {
        let totalFactors: IStatsModel = <IStatsModel>{
            atk: 1,
            spa: 1,
            def: 1,
            spd: 1,
        ...
        for (let fieldBoost of model.fieldBoosts.filter((fb) => fb.active)) {
    ...
    private getStatModifiersSubTotal(
        subTotal: IStatsModel,
        statModifiersExtent: IStatsModel
    ): IStatsModel {
        return <IStatsModel>{
    ...
    private getFieldBoostsSubTotal(
        subTotal: IStatsModel,
    ...
    private getBaseStatPartialValue(
    ...
    private getHpBaseStatPartialValue(
        baseStatValue: number,
        level: number
    ): number {
    ...
    private getNaturePartialValue(
        natureInfluence: number,
        subTotal: number
    ): number {
    ...
    private getAbilityPartialValue(
        abilityBoost: number,
        subTotal: number
    ): number {
    ...
    private getStatModifierPartialValue(
        statModifierStage: number,
        subTotal: number
    ): number {
        return (
            (this.getFactorForStatModifier(statModifierStage) - 1) * subTotal
        );
    }
    ...
    private getFieldBoostPartialValue(
        totalFieldBoostFactor: number,
        subTotal: number
    ): number {
